 ...
 * installation:
 * this is desgiend to be embedded in a storyline block in rise
 * in storyline, create a text entry field and set the variable to 'TextEntry'
 * add this script as a javascript trigger on the slide timeline start and set the action to 'load'
 * then add a button to the slide and add this script as a javascript trigger on the button click and set the action to 'store' or 'download'
 */

// This script is generic and exeucutes on timeline start AND on submit button click AND in the downloader

const Action = 'store';

// 'load' - on timeline start to repopulate text entry
// 'store' - on save button click to store text entry
// 'download' - on download button click to download pdf
...
// generate a temporary identifier for the session (kind is any string)
...
// find the runtime window where Rise is currently running
...
// find the LMS API window where the scorm runtime is running
...
// get the local time for timestamping the data
...
// get the instance identifier for the current session
function getInstance() {
    if (isCrossDomain()) return createDigest(window.location.href);
    return createDigest(window.parent.location.href);
}

// turn the player background transparent
function PATCH_STYLE() {
...
// some code can only run in certain conditions
if (window.parent !== window.self && !isCrossDomain()) {
    ...
    try {
        if (runtimeWindow.hasOwnProperty('courseData')) {
        	...
        	if (runtimeWindow.querySelector('.nav-sidebar-header__title')) {
	            courseTitle = runtimeWindow.querySelector('.nav-sidebar-header__title').textContent;
	            titleSource = 'sidebarHeader';
	        } else {
	            courseTitle = runtimeWindow.document.title.split(/\s\-\s(.*)/s).pop();
				titleSource = 'runtimeDocTitle';
			}
    ...
    try {
        ...
        // ometimes matches parent.document.title
    } catch (e) {
    }
...
// ask the server to generate a pdf and download it
function DOWNLOAD_DATA() {
    SERVER.then((endpoint) => {
        ...
        setTimeout(() => { a.remove(); }, 100); // remove the download link
        SERVER.then((endpoint) => {
