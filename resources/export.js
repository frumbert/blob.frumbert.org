...
// reference to the storyline player
const STORYLINE = GetPlayer();
const BLOCK = window.name; // the name of the iframe, which is the block id

async function GetServerLocation() {
    return new Promise((resolve, reject) => {
        fetch('/local/blobfiledb/', { method: 'HEAD' })
        .then((response) => {
            if (response.ok) {
                resolve('/local/blobfiledb/');
            } else {
                resolve('https://blob.frumbert.org');
            }
        }).catch((error) => {
            resolve('https://blob.frumbert.org');
        });
...
let SERVER = await GetServerLocation();
...
Promise.resolve( createDigest(myName + myId) )
.then((digest) => {
    let url = new URL(`${SERVER}/${digest}/${contextId}/download`);
    ...
    let url = new URL(`${SERVER}/${digest}/${contextId}/cleanup`);
    return fetch(url.toString(), {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
